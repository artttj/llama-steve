...
function renderOutput(r) {
  ...
  return `<p class="cmd"><span class="p">➜</span> llama-steve teardown <b>./${esc(r.product.name)}</b></p>
    <p class="out-meta">${esc(`scanned ${r.product.scope || 'the repo'} · ${r.ideasConsidered || 0} ideas from /10x · ${(r.killed || []).length} cut · ${r.features.length} shipped · 1 verdict`)}</p>
...
export function renderReport(r, { mode = 'report' } = {}) {
  ...
  const finalBlock = mode === 'landing'
    ? `<h2>Stop adding. Start cutting.</h2>
       ...
       <div class="install"><span>/plugin install <b style="color:var(--green)">llama-steve</b></span><span class="copy">COPY</span></div>`
    : `<h2>That's the teardown.</h2>
       ...
       <div class="install"><span>llama-steve&nbsp;<b style="color:var(--green)">serve --open</b>&nbsp;to re-run</span></div>`
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>LLama Steve — ${esc(r.product.name)}</title>
<link rel="stylesheet" href="assets/llama-steve.css">
</head>
<body>
<nav><div class="wrap">
  <div class="brand"><span class="dot">S</span> LLama&nbsp;Steve</div>
  <div class="nav-links"><a href="#model">The mind</a><a href="#output">The output</a><a href="#verdict">The verdict</a></div>
  ${navCta}
</div></nav>

<header>
  <p class="eyebrow">INTRODUCING LLAMA STEVE</p>
  <h1>Your roadmap has <span id="roadmapCount" data-count>1,000</span> features.<span class="sub">He ships 3.</span></h1>
  <p class="lede">Point him at your product. He runs a 10× teardown, says no to <span id="sayNoCount">997</span> good ideas, and hands back the handful worth your weekend — with a verdict you'll actually feel.</p>
  ...
  <div class="portrait-stage">
    ...
    <span class="portrait-tag">"Real artists sh<i>ee</i>p."</span>
  </div>
</header>
...
<section class="ad" id="model"><div class="wrap"><div class="ad-card">
  <div class="ad-body">
    ...
    <p class="ad-p">Most "act like Steve Jobs" skills are a sentence and a vibe. This one was distilled from the <b>Steve Jobs archive</b> — the Isaacson biography, the 2005 Stanford address, the Lost Interview, the D-Conference talks, <b>Make Something Wonderful</b>, and 30+ primary sources — into six working mental models, eight decision heuristics, and a full expression DNA. So when he says <b>no</b>, it's the same instinct that killed the hardware keyboard and put a thousand songs in your pocket. Not a costume. A judgment engine.</p>
  </div>
  <div class="ad-stats">
...
<section id="output"><div class="wrap">
  <div class="out-head">
    ...
    <p class="sec-sub">Runs <b>/10x</b> for the game-changing ideas, then puts every one through the Jobs filter. Ranked by least effort, most benefit. 🔥 = how hard it slaps.</p>
  </div>
  <div class="terminal">
...
<section class="verdict" id="verdict"><div class="wrap">
  <p class="label" style="text-align:center;margin-bottom:34px">The verdict — in his voice, run through /humanizer</p>
  <div class="verdict-card">
    <span class="${stampClass}">${esc(r.verdict?.stamp || 'SHEEP IT')}</span>
    <img class="vphoto" src="assets/llama-steve.png" alt="LLama Steve">
    <div>
      <p class="vquote">${esc(r.verdict?.quote || '')}</p>
      <p class="vby">— LLama Steve, after the teardown</p>
      <span class="humanized"><span class="pip"></span> written through /humanizer — no AI tells, no "delve," no hedging</span>
    </div>
  </div>
</div></section>

<section class="final" id="get"><div class="wrap">${finalBlock}</div>
  <footer><div class="fl"><a href="https://github.com/artttj/llama-steve">GitHub</a><a href="#model">The mind</a><a href="https://github.com/artttj/llama-smith">llama-smith</a><a href="https://artttj.de/llama-steve">Live demo</a></div>
...
<script>
(function(){
  var road=document.getElementById('roadmapCount'),no=document.getElementById('sayNoCount');
  if(!road)return;
  var target=1100+Math.floor(Math.random()*800);
  var fmt=function(n){return n.toLocaleString('en-US')};
  function set(n){road.textContent=fmt(n);if(no)no.textContent=fmt(Math.max(0,n-3))}
  if(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches){set(target);return}
  var dur=1500,t0=null;
  function tick(ts){if(!t0)t0=ts;var p=Math.min(1,(ts-t0)/dur);var e=p<0.5?2*p*p:1-Math.pow(-2*p+2,2)/2;set(Math.round(target*e));if(p<1)requestAnimationFrame(tick);else set(target)}
...
export function buildReport(r, outDir, opts = {}) {
  ...
  writeFileSync(join(outDir, 'index.html'), renderReport(r, opts))
