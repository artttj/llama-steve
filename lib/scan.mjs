...
export function repoIdentity(root) {
  ...
  try {
    ...
    const ssh = remote.match(/^[^@]+@([^:]+):(.+)$/) // git@host:owner/repo
    if (ssh) url = `https://${ssh[1]}/${ssh[2]}`
    else if (/^ssh:\/\//.test(remote)) url = remote.replace(/^ssh:\/\/[^@]+@/, 'https://')
    else if (remote) url = remote // already https/http or a usable URL
  } catch {}
